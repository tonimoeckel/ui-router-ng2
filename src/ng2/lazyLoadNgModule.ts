 ...
 * @internalapi
 */
export function applyNgModule(ng2Module: NgModuleRef<any>): LazyLoadResult {
  ...
  modules.reverse().forEach(module => {
      ...
      states.forEach(state => {
        ...
        uiRouter.stateRegistry.get(state.name).$$state().resolvables.push(Resolvable.fromData(NATIVE_INJECTOR_TOKEN, injector));
      });
  });
